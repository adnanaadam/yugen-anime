  ...
  if (status === "loading" || loading) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-foreground">Loading admin...</div>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-6xl px-4 py-8">
          ...
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <section className="rounded-xl border border-white/10 bg-white/5 p-4">
                <h2 className="text-sm font-semibold text-muted mb-3">Recent Users</h2>
                  ...
                  {stats.recentUsers.map((u) => {
                    ...
                    return (
                      <div key={user.id} className="flex items-center justify-between text-sm">
                        ...
                        <div className="text-xs text-muted">Lvl {user.level ?? 0}</div>
                      </div>
                    );
                  })}
                </div>
              ...
              <section className="rounded-xl border border-white/10 bg-white/5 p-4">
                <h2 className="text-sm font-semibold text-muted mb-3">Recent Feedback</h2>
                <div className="space-y-2">
                    ...
                    return (
                      <div key={feedback.id} className="text-sm">
                        <div className="flex items-center justify-between">
                          <span className="text-foreground capitalize">{feedback.category}</span>
                          <span className="text-xs text-muted">{new Date(feedback.createdAt || "").toLocaleDateString()}</span>
                        </div>
                        <p className="text-xs text-muted mt-0.5 line-clamp-2">{feedback.message}</p>
                      </div>
                    );
                  })}
                </div>
        ...
        {tab === "notifications" && (
          <section className="rounded-xl border border-white/10 bg-white/5 p-4 max-w-xl">
            <h2 className="text-sm font-semibold text-muted mb-3">Send System Notification</h2>
            <form onSubmit={sendNotification} className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm text-foreground"
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm text-foreground h-24"
              ...
              {sendResult && <p className="text-xs text-muted mt-2">{sendResult}</p>}
            </form>
          </section>
        )}
        ...
        {tab === "users" && (
          <section className="rounded-xl border border-white/10 bg-white/5 p-4">
            <h2 className="text-sm font-semibold text-muted mb-3">Users ({users.length})</h2>
            <div className="space-y-2">
              {users.map((u) => {
        ...
        {tab === "feedback" && (
          <section className="rounded-xl border border-white/10 bg-white/5 p-4">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-semibold text-muted">Feedback ({feedbackTotal})</h2>
              <select
              ...
              {feedbackItems.map((f) => {
                ...
                return (
                  <div key={item.id} className="rounded-lg border border-white/5 bg-black/20 px-3 py-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-medium text-muted capitalize">{item.category}</span>
                      <span className="text-[10px] text-muted">{new Date(item.createdAt || "").toLocaleString()}</span>
                    </div>
